...
public sealed class QuestRewardExp
{
    ...
    private ExpChip CreateChip(ExpKind kind, long amount, JobType? job = null, Factions? faction = null)
    {
        ...
        chip.Label = new Label(chip.Container, "QuestRewardExpLabel")
        {
            FontName = FontName,
            FontSize = FontSize,
            Text = FormatExpText(kind, amount, job, faction),
        };
    ...
    private static string FormatExpText(ExpKind kind, long amount, JobType? job, Factions? faction)
    {
        switch (kind)
        {
            case ExpKind.Player:
            ...
            case ExpKind.Job:
                return $"{Strings.FormatQuantityAbbreviated(amount)} ({job})";
            case ExpKind.Guild:
                return Strings.FormatQuantityAbbreviated(amount);
            case ExpKind.Faction:
                return $"{Strings.FormatQuantityAbbreviated(amount)} ({faction})";
            default:
    ...
    private sealed class ExpChip
    {
        ...
        {
            Amount = newAmount;
            Label.Text = QuestRewardExp.FormatExpText(Kind, Amount, Job, Faction);
        }
    }
