     ...
     ItemType? type = null
        ...
        public static bool TryListItem(Player seller, Item item, int quantity, int pricePerUnit, bool autoSplit = false, int slotIndex = -1)
        {
            ...
            if (cooldown > 0 && seller.LastMarketAction.HasValue)
            {
                ...
                {
                    var remainingCooldown = (int)Math.Ceiling(cooldown - elapsed.TotalSeconds); // Renamed variable to 'remainingCooldown'
                    PacketSender.SendChatMsg(seller, $"⏳ Debes esperar {remainingCooldown}s antes de realizar otra operación en el mercado.", ChatMessageType.Error, CustomColors.Alerts.Error);
                    return false;
                }
            }
