...
public partial class BankInterface<TSlot> : IBankInterface where TSlot : Item, ISlot
{
    ...
    private static int CompareBankItems(TSlot? currentItem, TSlot? comparedItem)
    {
        ...
        {
            if (Options.Instance.Items.ItemSubtypes.TryGetValue(x.ItemType, out var subtypes))
            {
                var xIndex = subtypes.FindIndex(s => s.Equals(x.Subtype, StringComparison.OrdinalIgnoreCase));
                var yIndex = subtypes.FindIndex(s => s.Equals(y.Subtype, StringComparison.OrdinalIgnoreCase));
        ...
        switch (x.ItemType)
        {
            ...
            case ItemType.Event:
                return y.ItemType == ItemType.Currency || y.ItemType == ItemType.Equipment || y.ItemType == ItemType.Bag ||
                       y.ItemType == ItemType.Resource
                    ? -1
                    : 1;
            case ItemType.Spell:
                return y.ItemType == ItemType.Currency || y.ItemType == ItemType.Equipment || y.ItemType == ItemType.Bag ||
                       y.ItemType == ItemType.Resource || y.ItemType == ItemType.Event
                    ? -1
                    : 1;
            case ItemType.None:
