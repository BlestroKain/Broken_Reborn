    ...
    internal sealed partial class LogicService
    {
        ...
        internal sealed class LogicThread : Threaded<ServerContext>
        {
            ...
            private void AddToQueue(MapProcessingLayer mapProcessingLayer)
            {
                ...
                mapProcessingLayer.LastRequestedUpdateTime = Globals.Timing.Milliseconds - Options.Instance.Processing.MapUpdateInterval;
            }
            ...
            private void UpdateMap(MapProcessingLayer layer, bool onlyProjectiles)
            {
                try
                {
                    if (onlyProjectiles)
                    {   
                        layer.UpdateProjectiles(Globals.Timing.Milliseconds);
                    ...
                    else
                    {
                        if (Options.Instance.Metrics.Enable)
                        {
                            ...
                            layer.Update(Globals.Timing.Milliseconds);
